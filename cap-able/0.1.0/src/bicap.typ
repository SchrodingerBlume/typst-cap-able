...
#let bicap(
  ...
  /// `table.header(repeat: true)` 实现，复用与 `captab(repeat-caption: true)` 同款的
  ...
  /// `query(label) + here().page()` mechanism from `captab(repeat-caption: true)` so the
  ...
  /// 仅 `kind == "table"` 生效，与 `repeat-caption` 互相独立。
  ///
  ...
  /// Only active when `kind == "table"`; independent of `repeat-caption`.
  /// -> auto | bool | int
  repeat-header: auto,
  ...
  context {
    ...
    // ── repeat-caption 启用条件 / repeat-caption activation ──
    ...
    let cap-in-body = (repeat-caption
                       and kind == "table"
                       and caption != none
                       and refer-to == none
                       and body != none)
    ...
    if caption != none or refer-to != none {
      ...
      // 没有题注但有 body：直接输出 body / No caption, only body: just emit body
      presented-body
    }
  }
}
