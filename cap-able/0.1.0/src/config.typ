...
#let table-style-config = state("table-style-config", (
  ...
  repeat-caption: false,      // 跨页时是否重复题注（true 时题注会在每页表头上方再渲染一次）/ Repeat caption on each page
...
#let captab-style(
  ...
  repeat-caption: auto,       // bool：跨页时是否在每页表头上方重复题注 / Repeat caption on every page
  ...
  context {
    ...
    if repeat-caption != auto { new.insert("repeat-caption", repeat-caption) }
...
#let cap-style(
  ...
  caption-position: auto,
  // 是否使用三线表样式（仅作用于表格；图片配置无此项）
  // Three-line table mode (table-only; figure config has no equivalent)
  three-line-table: auto,
  // 单元格内边距（cell-inset 与 inset 互为别名；仅作用于表格）
  // Cell padding (cell-inset and inset are aliases of each other; table-only)
  cell-inset: auto,
  inset: auto,
  it,
) = {
