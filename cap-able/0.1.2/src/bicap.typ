...
#let bicap(
  ...
  /// -> bool
  ///
  /// 首选名 `show-continued-caption`（与 show-subcaption 等命名统一）。
  /// `continued-caption` 是旧名，0.1.x 仍兼容，将在 0.2.0 移除。
  /// Preferred name is `show-continued-caption`; `continued-caption` is a deprecated
  /// alias kept through 0.1.x and removed in 0.2.0.
  /// -> auto | bool
  ...
  context {
    ...
    // 首选名 show-continued-caption > 旧名 continued-caption（0.2.0 移除）> false
    // Preferred show-continued-caption > deprecated continued-caption (removed in 0.2.0) > false
    let final-continued-caption = if show-continued-caption != auto {
      show-continued-caption
    } else {
      continued-caption
    }
    let cap-in-body = (final-continued-caption
                       and kind == "table"
                       and caption != none
                       and refer-to == none
                       and body != none)
    ...
    // 浮动元素与正文的间距走 place 的 clearance（block above/below 对 float 无效）。
    // top → 正文在下，用 below 间距；bottom → 正文在上，用 above 间距；auto → below。
    // 间距字段按 kind 选：figure 用 figure-above/below，table 用 caption-above/table-below。
    // issue #14。
    // Float-to-body gap via place's `clearance`. top → below gap, bottom → above gap,
    // auto → below. Spacing fields keyed by kind. Issue #14.
    let _maybe-float(content) = if final-placement != none {
      let above-k = if kind == "figure" { "figure-above" } else { "caption-above" }
      let below-k = if kind == "figure" { "figure-below" } else { "table-below" }
      let cl-key = if final-placement == bottom { above-k } else { below-k }
      let cl = final-config.at(cl-key, default: 0pt)
      place(_normalize-placement(final-placement), float: true, clearance: cl, content)
    } else {
      content
    }
    ...
    if caption != none or refer-to != none {
      if body == none {
        ...
        // sticky: true 防止 caption 与 body 跨页分离（issue #16）。排在前面的那块
        // 设 sticky——top 时 caption 粘住下方 body，bottom 时 body 粘住下方 caption。
        // sticky: true keeps caption and body from splitting across a page break (issue
        // #16). The first block gets sticky — top: caption sticks to the body below;
        // bottom: body sticks to the caption below.
        let composed = if main-align-resolved.kind == "text" {
          ...
          if final-position == top {
            ...
            block(width: 100%, sticky: true, align(center, presented-body))
            v(gap)
            cap-line
          }
        } else {
          align(center)[
              ...
              block(sticky: true, presented-body)
              v(gap)
              std.align(main-align-resolved.value, caption-content)
            }
          ]
