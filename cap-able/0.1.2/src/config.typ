...
#let captab-style(
  ...
  show-continued-caption: auto,  // bool：首选名，跨页时是否在每页表头上方重复题注 / Preferred name
  continued-caption: auto,       // bool：旧名，0.1.x 兼容，0.2.0 移除 / Deprecated alias, removed in 0.2.0
  ...
  context {
    ...
    // 旧名先写，首选名后写覆盖；两者都写进同一个 state key "continued-caption"
    // Apply deprecated alias first, then let the preferred name override; both map to
    // the same internal state key "continued-caption".
    if continued-caption != auto { new.insert("continued-caption", continued-caption) }
    if show-continued-caption != auto { new.insert("continued-caption", show-continued-caption) }
