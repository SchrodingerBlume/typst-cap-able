...
#let capfig(
  ...
  context {
    ...
    if final-placement != none {
      ...
      // 浮动元素与正文之间的间距走 place 的 clearance（外层 v() 对 float 无效）。
      // 浮动只有"朝向正文的一侧"间距有意义：top → 用 figure-below；bottom → figure-above；
      // auto 落点未知，取 figure-below 兜底。issue #14。
      // Float-to-body spacing uses place's `clearance` (outer v() has no effect on floats).
      // A float only has one meaningful gap (toward the body): top → figure-below,
      // bottom → figure-above, auto → figure-below as fallback. Issue #14.
      let float-clearance = if final-placement == bottom {
        final-figure-above
      } else {
        final-figure-below
      }
      place(aligned-placement, float: true, clearance: float-clearance, composed)
    } else {
      // 图片上方间距（通过 v() 实现）
