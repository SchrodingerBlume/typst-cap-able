...
#let captab(
  ...
  breakable: auto,
  repeat-header: auto,
  // 首选名，与 show-subcaption 等命名一致 / Preferred name (matches show-subcaption etc.)
  show-continued-caption: auto,
  // 旧名，0.1.x 仍兼容，将在 0.2.0 移除 / Deprecated alias, removed in 0.2.0
  ...
  context {
    ...
    // 优先级：show-continued-caption（首选名）> continued-caption（旧名，0.2.0 移除）> 全局 state > false
    // Precedence: show-continued-caption (preferred) > continued-caption (deprecated, removed in 0.2.0) > state > false
    let final-continued-caption = if show-continued-caption != auto {
      show-continued-caption
    } else if continued-caption != auto {
      continued-caption
    } else {
      config.at("continued-caption", default: false)
    }
    ...
    if caption != none {
      ...
      // sticky: true 防止 caption 与表体跨页分离（issue #16）。
      // 排在前面的那一块设 sticky——它若将成为页面最后一个元素，会连同后续内容一起
      // 移到下一页：top 时是 caption 粘住下方的表；bottom 时是表粘住下方的 caption。
      // 表体设了 sticky 仍可内部跨页（block 默认 breakable: true），只是末段不会
      // 把 caption 甩在后面。
      // sticky: true keeps the caption from being orphaned across a page break (issue #16).
      // The *first* block gets sticky — if it would be the last thing on a page, it (and
      // the content it sticks to) moves to the next page together: for top the caption
      // sticks to the table below; for bottom the table sticks to the caption below.
      // A sticky table still breaks internally (block is breakable by default); only its
      // tail will not leave the caption behind.
      let composed = if main-align-resolved.kind == "text" {
        ...
        if final-caption-position == bottom {
          block(width: 100%, sticky: true, std.align(center, table-body))
          gap
          cap-line
        } else {
          block(width: 100%, sticky: true, std.align(main-align-resolved.value, caption-content))
          gap
          table-centered
        }
      } else {
        ...
        let group = block(width: inner-width)[
          #if final-caption-position == bottom {
            block(sticky: true, table-body)
            gap
            std.align(main-align-resolved.value, caption-content)
          } else {
            block(sticky: true, std.align(main-align-resolved.value, caption-content))
            gap
            table-body
          }
        ]
      ...
      // 浮动元素与正文的间距走 place 的 clearance（block 的 above/below 对 float 无效）。
      // top → 正文在下，用 table-below；bottom → 正文在上，用 caption-above；auto 取 table-below。
      // issue #14。
      // Float-to-body spacing via place's `clearance` (block above/below has no effect on
      // floats). top → table-below; bottom → caption-above; auto → table-below. Issue #14.
      let _float-clearance(p) = if p == bottom {
        config.caption-above
      } else {
        config.table-below
      }
      if final-placement != none {
        place(_normalize-placement(final-placement), float: true,
              clearance: _float-clearance(final-placement), outer-block)
      } else {
        outer-block
      }
      ...
      if final-placement != none {
        place(_normalize-placement(final-placement), float: true,
              clearance: _float-clearance(final-placement), bare)
      } else {
        bare
      }
      if should-indent { _fakepar }
