...
- 修复 `captab-style` / `capfig-style` 在嵌套 `context` 内被二次套用时，触发 Typst `"layout did not converge within 5 attempts"` 警告的问题。根因：原 patch 实现走 `context { state.get(); modify; state.update(new) }` 的 read-then-write，Typst layout convergence 检测会把每次迭代视作潜在状态变更而无法收敛。改用 `state.update(prev => modify(prev))` 函数式更新，updater 是纯函数、不读 state，Typst 能直接判幂等。下游论文模板中复现报告，PDF 渲染本来就正确，但每次编译都吐警告。
